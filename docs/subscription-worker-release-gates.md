...
| Idempotent DAIA delivery | Changed retry rejected, exact retry retained one stored result; source matched independent evaluator | Native Codex MCP delivery and exact retry now have a [live subscription result](research/native-subscription-mcp-delivery-2026-09-13.json), with independent evaluation of the stored artifact. Still prove crash recovery through a newly created guest without changing consent or claiming new work |
