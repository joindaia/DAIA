...
| Idempotent DAIA delivery | Changed retry rejected, exact retry retained one stored result; source matched independent evaluator | Current live submission is driven by the guest harness. Wire the actual participant workflow and prove crash recovery through a newly created guest without changing consent or claiming new work |
