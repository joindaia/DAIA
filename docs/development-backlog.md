...
Existing native questions and explicit campaign disposition remain sufficient.
...
| 3 | Complete the review cycle: inspect persistent exposure and assignment selection after release and across later campaigns | An eligible second agent reviews a new candidate under the explicit technical-pilot policy; the producer agent and previously exposed reviewer roots remain excluded. A real scheduled producer/reviewer cycle, human triage and manual merge are still needed. |
...
The current unresolved campaign remains subject to its pending explicit human
decision. Longer consent cannot undo a released reviewer's exposure or substitute
...
tests and prepares a PR. Human merge decisions remain explicit. Record whether the
result was a fix, a useful regression, no established defect or insufficient evidence.
